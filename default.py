...
addon_id = 'plugin.program.silenceromwizard'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.program.silenceromwizard'
AddonTitle="SilenceROM Wizard"
...
PATH = "SilenceROM Wizard"            
BASEURL = "https://archive.org"
H = 'http://'
EXCLUDES     = ['plugin.program.silenceromwizard','script.module.addon.common']

def INDEX():
    addDir('Install SilenceROM',BASEURL,2,ART+'icon.png',FANART,'')
    addDir('Maintenance',BASEURL,3,ART+'icon.png',FANART,'')
    #addDir('Changelog',BASEURL,9,ART+'icon.png',FANART,'')
    setView('Program', 'INFO')

def BUILDMENU():
    addDir('SilenceROM Latest',BASEURL+'/download/SilenceROM_Source/SilenceROM_Latest.zip',5,ART+'icon.png',FANART,'')
    #addDir('Install Adult Content',BASEURL+'/download/SilenceROM_Source/SilenceROM_Adult.zip',10,ART+'icon.png',FANART,'')
    setView('Program', 'INFO')
...
def MAINTENANCE():
    ...
    #addDir('iVue TV Guide Reset','url',11,ART+'icon.png',FANART,'')
    setView('program', 'INFO')
...
def Changelog():
  TextBoxes('Complete Changelog','Place Changelog here!')
...
def DELETEIVUEDB():
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("SilenceROM Maintenance", "Database Files Removed, Please reload TV Guide", "[COLOR yellow]Brought To You By SilenceROM Wizard[/COLOR]")
...
def WIZARD(name,url,description):
    ...
    dp.create("SilenceROM Wizard","Downloading ",'', 'Please Wait')
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("SilenceROM Wizard", "To save changes you now need to force close Kodi, Press OK to force close Kodi")
    killxbmc()
...
def ADDONWIZARD(name,url,description):
    ...
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("SilenceROM Wizard", "Please restart kodi for changes To Take Effect","[COLOR yellow]Brought To You By SilenceROM Wizard[/COLOR]")
            ...
            if file_count > 0:
                ...
                if dialog.yesno("Delete Package Cache Files", str(file_count) + " files found", "Do you want to delete them?"):
                    ...
                    for d in dirs:
                        shutil.rmtree(os.path.join(root, d))
                    dialog = xbmcgui.Dialog()
                    dialog.ok("SilenceROM Wizard", "Packages Successfuly Removed", "[COLOR yellow]Brought To You By SilenceROM Wizard[/COLOR]")
    except: 
        dialog = xbmcgui.Dialog()
        dialog.ok("SilenceROM Wizard", "Sorry we were not able to remove Package Files", "[COLOR yellow]Brought To You By SilenceROM Wizard[/COLOR]")
...
def deletecachefiles(url):
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("SilenceROM Wizard", " All Cache Files Removed", "[COLOR yellow]Brought To You By SilenceROM Wizard[/COLOR]")
            ...
            for root, dirs, files in os.walk(xbmcPath,topdown=True):
                ...
                for name in dirs:
                    try: os.rmdir(os.path.join(root,name))
                        ...
                        plugintools.log("Error removing "+root+" "+name)
            if not failed: plugintools.log("freshstart.main_list All user files removed, you now have a clean install"); plugintools.message(AddonTitle,"The process is complete, you're now back to a fresh Kodi configuration with SilenceROM Wizard!","Please reboot your system or restart Kodi in order for the changes to be applied.")
            else: plugintools.log("freshstart.main_list User files partially removed"); plugintools.message(AddonTitle,"The process is complete, you're now back to a fresh Kodi configuration with SilenceROM Wizard","Please reboot your system or restart Kodi in order for the changes to be applied.")
